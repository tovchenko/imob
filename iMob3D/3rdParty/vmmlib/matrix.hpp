...
public:
    ...
    void zero();
	void identity( typename enable_if< M == N >::type* = 0 );
    ...
    */
	...
	template< typename TT >
	inline void set_lookAt( const vector< 3, TT >& eye_, const vector< 3, TT >& dir_, const vector< 3, TT >& up_, 
		typename enable_if< M == N && M == 4, TT >::type* = 0 );
...
make_identity( matrix< M, N, T >& matrix_ )
{
...
template< size_t M, size_t N, typename T >
void
matrix< M, N, T >::identity( typename enable_if< M == N >::type* )
{
	make_identity(*this);
}
...
void
matrix< M, N, T >::
set_lookAt( const vector< 3, TT >& eye_, const vector< 3, TT >& dir_, const vector< 3, TT >& up_, 
	typename enable_if< M == N && M == 4, TT >::type* )
{
	vector< 3, TT > z_axis = dir_ - eye_;
	z_axis.normalize();
	z_axis.negate();
	
	vector< 3, TT > x_axis;
	x_axis.cross(up_, z_axis);
	x_axis.normalize();
	
	vector< 3, TT > y_axis;
	y_axis.cross(z_axis, x_axis);
	
	array[ 0 ] = x_axis.at( 0 );	array[ 1 ] = y_axis.at( 0 );	array[ 2 ] = z_axis.at( 0 );
	array[ 4 ] = x_axis.at( 1 );	array[ 5 ] = y_axis.at( 1 );	array[ 6 ] = z_axis.at( 1 );
	array[ 8 ] = x_axis.at( 2 );	array[ 9 ] = y_axis.at( 2 );	array[ 10 ] = z_axis.at( 2 );
	
	array[ 12 ] = -dot(x_axis, eye_);
	array[ 13 ] = -dot(y_axis, eye_);
	array[ 14 ] = -dot(z_axis, eye_);
